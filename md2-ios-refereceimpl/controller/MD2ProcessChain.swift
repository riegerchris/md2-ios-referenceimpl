...
class MD2ProcessChain {

    let processChainSignature: String
    
    var steps: Array<MD2ProcessChainStep> = []
    ...
    func addProcessChainStep(stepSignature: String, viewName: String) -> MD2ProcessChainStep {
        let step = MD2ProcessChainStep(stepSignature: stepSignature, processChain: self, viewName: viewName)
        steps.append(step)
        return step
    }
    ...
    func goTo(step: MD2ProcessChainStep) {
