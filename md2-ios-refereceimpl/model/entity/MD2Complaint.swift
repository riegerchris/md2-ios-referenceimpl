...
class MD2Complaint: MD2EntityType {
    ...
    required init() {
        ...
        containedTypes["loc"] = MD2Address()
        containedTypes["descriptions"] = MD2String()
        containedTypes["feedback"] = MD2String()
        containedTypes["status"] = MD2ComplaintStatus()
    }
    
    convenience init(md2Entity: MD2Complaint) {
        self.init()
    ...
    func clone() -> MD2Type {
        return MD2Complaint(md2Entity: self)
    }
    ...
    func equals(value : MD2Type) -> Bool {
        if !(value is MD2Complaint) {
            return false
        }
        
        var isEqual = true
        
        for (typeName, typeValue) in (value as! MD2Complaint).containedTypes {
